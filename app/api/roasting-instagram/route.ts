...
function isEnglish(response: string) {
  const englishPatterns = [
    ...
    /Note:/i,
    /the word/i,
    /\b(the|and|is|in|to|of|with|for|on|you|this|that|it|we|I)\b/i, // General English words
    /Please note that/i,
    /managed by/i,
    /sarcasm/i,
    /irony/i,
...
async function generateFilteredContent(model: string, prompt: string) {
  ...
  } while (containsFilteredWords(result ?? "") || isEnglish(result ?? "")); 
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    let igData = await axios.get(
      `https://api.sanzy.co/api/igstalk?username=${query}`
    );
    ...
    var prompt = `berikan roasting dengan kejam, menyindir, serta menyakitkan dalam bahasa gaul untuk profile instagram berikut: ${query}. Berikut detailnya: "${JSON.stringify(
      DataIg
    )}"`;
